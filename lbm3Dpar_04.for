...
C-----
      ...
      ni = 600   !600    ! lattice nodes
      nj = 100   !100    ! lattice nodes
      nk = 200   !200    ! lattice nodes
...
C=====change obs to match lcd array from ufocfd
      IF (.true.) THEN
        write(6,*) 'read obstacle from file'
...
C-------write iterations
        if (mod(its,writeits).eq.0) then
          write(6,*) "its vel rho ",its,vel(100,50,50,1),rho(100,50,50)
        endif
...
C---------make outfile
          if (iseq.eq.0) then
            outfile = './vtkfiles/lbm3d.vtk'
          elseif (iseq.eq.1) then
            write(unit=string, fmt='(I6.6)') its
            outfile = './vtkfiles/lbm3d_'//string//'.vtk'
          endif
...
C---------start binary VTK file
          if (.true.) then
          write(6,*)"write binary vtk file"
          OPEN(unit=20, file=outfile, form='unformatted',
     &    access='stream',status='replace',convert="big_endian") ! write binary vtk 
C         can use compile flag -fconvert=big-endian
